...
namespace colorutil
{
    ...
    calculate_CIEDE2000(const Lab& color_1, const Lab& color_2, double k_L = 1.0, double k_C = 1.0, double k_H = 1.0);
    ...
    constexpr double pow_25_7 = 6103515625.0; // std::pow(25.0, 7.0)

    inline double calculate_CIEDE2000(const Lab& color_1, const Lab& color_2, double k_L, double k_C, double k_H)
    {
        ...
        // Step 1
        const double a1_sq = a_1 * a_1;
        const double b1_sq = b_1 * b_1;
        const double a2_sq = a_2 * a_2;
        const double b2_sq = b_2 * b_2;

        const double C_1_ab   = std::sqrt(a1_sq + b1_sq);
        const double C_2_ab   = std::sqrt(a2_sq + b2_sq);
        ...
        // Squared terms for C_prime calculations
        const double a1_prime_sq = a_1_prime * a_1_prime;
        const double a2_prime_sq = a_2_prime * a_2_prime;

        const double C_1_prime = std::sqrt(a1_prime_sq + b1_sq);
        const double C_2_prime = std::sqrt(a2_prime_sq + b2_sq);
        ...
        const double h_prime_bar_diff = (h_prime_bar - 275.0) / 25.0;
        const double delta_theta      = 30.0 * std::exp(-h_prime_bar_diff * h_prime_bar_diff);
        ...
        // Manual expansion of quadratic terms
        const double L_prime_bar_50    = L_prime_bar - 50.0;
        const double L_prime_bar_50_sq = L_prime_bar_50 * L_prime_bar_50;

        const double S_L = 1.0 + (0.015 * L_prime_bar_50_sq) / std::sqrt(20.0 + L_prime_bar_50_sq);
